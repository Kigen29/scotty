...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Block sending to leads whose contact details were generated rather than
    // sourced. This is the manual path, so the message is written for a human.
    if (lead.verification_state !== "verified") {
      const why = lead.verification_state === "rejected"
        ? "This lead's details were generated by a model, not found from a real source, so it cannot be contacted."
        : "This lead's provenance has not been confirmed. Re-discover it from a real search result before contacting it.";
      return new Response(JSON.stringify({ error: why, verification_state: lead.verification_state }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
