...
-- ============================================================
-- 1. Recreate ALL RLS policies as PERMISSIVE
-- ============================================================
...
CREATE POLICY "Admins and owners can manage members" ON public.team_members AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (
...
CREATE POLICY "Admins and owners can update members" ON public.team_members AS PERMISSIVE FOR UPDATE TO authenticated USING (
  is_team_member(auth.uid(), team_id) AND (has_team_role(auth.uid(), team_id, 'owner'::team_role) OR has_team_role(auth.uid(), team_id, 'admin'::team_role))
);

DROP POLICY IF EXISTS "Admins and owners can remove members" ON public.team_members;
CREATE POLICY "Admins and owners can remove members" ON public.team_members AS PERMISSIVE FOR DELETE TO authenticated USING (
  is_team_member(auth.uid(), team_id) AND (has_team_role(auth.uid(), team_id, 'owner'::team_role) OR has_team_role(auth.uid(), team_id, 'admin'::team_role))
);

-- ============================================================
-- 2. Add UNIQUE constraint on team_members to prevent duplicates
-- ============================================================
ALTER TABLE public.team_members ADD CONSTRAINT unique_user_team UNIQUE (user_id, team_id);
